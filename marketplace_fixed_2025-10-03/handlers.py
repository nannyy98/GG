...
def btn_texts(key):
    ...
    return {t(key, language='ru'), t(key, language='uz')}


class MessageHandler:
    def _get_language(self, message):
        telegram_id = message['from']['id']
        user_data = self.db.get_user_by_telegram_id(telegram_id)
        if user_data and user_data[0][5]:
    ...
    def handle_message(self, message):
        ...
        try:
            ...
            elif text in btn_texts('btn_catalog') or text == '🛍 Перейти в каталог':
                self.show_catalog(message)
            elif text == '🔙 К категориям':
                self.show_catalog(message)
            elif text.startswith('🛍 '):
                self.handle_product_selection(message)
            elif text in btn_texts('btn_cart'):
                self.show_cart(message)
            elif text in btn_texts('btn_orders'):
                self.show_user_orders(message)
            elif text in btn_texts('btn_profile'):
                self.show_user_profile(message)
            elif text in btn_texts('btn_search'):
                self.start_product_search(message)
            elif text in btn_texts('btn_help'):
                self.handle_help_command(message, user_language)
            elif text == '⭐ Программа лояльности':
                self.bot.send_message(chat_id, '⏸ Функция временно отключена')
            elif False:  # disabled original block
                self.show_loyalty_program(message)
            elif text == '🎁 Промокоды':
                self.bot.send_message(chat_id, '⏸ Функция временно отключена')
            elif False:  # disabled original block
                self.show_available_promos(message)
            elif text in btn_texts('btn_main'):
    ...
    def handle_start_command(self, message):
        ...
        if user_data:
            ...
            self.bot.send_message(chat_id, welcome_text, create_main_keyboard(language=language))
        else:
    ...
    def handle_registration_language(self, message):
        ...
        if user_id:
            ...
            self.bot.send_message(chat_id, welcome_complete, create_main_keyboard(language=language))
    ...
    def handle_help_command(self, message, language='ru'):
        ...
        self.bot.send_message(chat_id, help_text, create_main_keyboard(language=language))
    ...
    def handle_payment_method_selection(self, message):
        ...
        if order_id:
            ...
            self.bot.send_message(chat_id, success_text, create_main_keyboard(language=language))
    ...
    def handle_callback_query(self, callback_query):
        ...
        try:
            ...
            if data == 'go_to_cart':
                # Переход в корзину из инлайн-кнопки
                msg = {'chat': {'id': chat_id}}
                self.show_cart(msg)
                return
            elif data == 'continue_shopping':
                # Продолжить покупки — показываем каталог
                msg = {'chat': {'id': chat_id}}
    ...
    def handle_unknown_command(self, message, language='ru'):
        ...
        if len(text) > 2 and not text.startswith('/'):
            ...
            if products:
                ...
                self.bot.send_message(chat_id, search_text, create_main_keyboard(language=language))
                return
        ...
        self.bot.send_message(chat_id, unknown_text, create_main_keyboard(language=language))